...
use state::{EXPANDED_HEIGHT, EXPANDED_WIDTH, HEIGHT, NotchState, WIDTH};
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let layer = layer_shell.create_layer_surface(&qh, surface, Layer::Top, Some("notch"), None);
    ...
    let pool = SlotPool::new((EXPANDED_WIDTH * EXPANDED_HEIGHT * 4) as usize, &shm)?;
